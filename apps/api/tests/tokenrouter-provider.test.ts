...
import { deleteProviderDB, upsertProviderDB } from "@codex/db";
...
test("saved TokenRouter connections use the official URL and bearer auth", async () => {
    const id = `tokenrouter_test_${Date.now()}`;
    ...
    assert.equal(requestUrl, "https://api.tokenrouter.com/v1/models");
...
test("processTokenRouterTokenImport creates and registers TokenRouter provider config", () => {
    ...
    assert.match(config.id, /^tokenrouter_\d+$/);
