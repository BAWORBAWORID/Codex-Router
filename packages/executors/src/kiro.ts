...
function parseFrames(bytes: Uint8Array): KiroEvent[] {
    ...
    while (offset < bytes.length) {
        if (bytes.length - offset < 12)
            throw new Error("Kiro EventStream ended with a truncated frame");
        const view = new DataView(bytes.buffer, bytes.byteOffset + offset, bytes.length - offset);
        const totalLength = view.getUint32(0, false);
        if (
            totalLength < 16 ||
            totalLength > MAX_FRAME_BYTES ||
            offset + totalLength > bytes.length
        )
            throw new Error("Kiro EventStream ended with a truncated frame");
...
export class KiroExecutor implements AIProvider {
    ...
    getOrderedBaseUrls(): string[] {
        ...
        const regionalize = (url: string): string =>
            region === "us-east-1"
                ? url
                : url.replace(/([a-z]+)\.us-east-1\.amazonaws\.com/, `$1.${region}.amazonaws.com`);
    ...
    buildRequest(req: ChatCompletionRequest): KiroRequest {
        ...
        const current =
            currentIndex >= 0
                ? messages.splice(currentIndex, 1)[0]!
        ...
        for await (const frame of iterEventStreamFrames(response.body, MAX_FRAME_BYTES)) {
            ...
            if (type === "assistantResponseEvent" || type === "codeEvent") {
                ...
                const content =
                    typeof value === "string"
                        ? value
                        : String(valueObject.text ?? valueObject.content ?? "");
                if (content) {
                    yield chunk(responseId, req.model, {
                        ...(first ? { role: "assistant" as const } : {}),
